...
import cn.lightfish.ast.IfStmnt;
...
public class Parser {
    ...
    public Parser ifElse(Parser ifStatement, Parser block, String ifString, String elseString) {
        elements.add(new IfStatementHelper(ifStatement, block, ifString, elseString));
        return this;
    }

    public Parser map(Element element) {
        elements.add(element);
        return this;
    }

    protected static class IfStatementHelper extends Element {
        Parser ifStatement;
        Parser block;
        String elseString;
        String ifString;

        ASTree ifStatementHelper(Lexer lexer) throws ParseException {
            ASTree root = ifStatement.parse(lexer);
            while (elseString.equals(lexer.peek(0).getText())) {
                if (ifString.equals(lexer.peek(1).getText())) {
                    lexer.read();
                    root = new IfStmnt(Arrays.asList(root.child(0), root.child(1), ifStatementHelper(lexer)));
                } else {
                    lexer.read();
                    root = new IfStmnt(Arrays.asList(root.child(0), root.child(1), block.parse(lexer)));
                    break;
                }
            }
            return root;
        ...
        protected IfStatementHelper(Parser ifStatement, Parser block, String ifString, String elseString) {
        ...
        protected void parse(Lexer lexer, List<ASTree> res)
                throws ParseException {
            res.add(ifStatementHelper(lexer));
        }

        protected boolean match(Lexer lexer) throws ParseException {
            return ifString.equals(lexer.peek(0).getText());
        }
    }
