...
import static com.pawandubey.griffin.model.Data.fileQueue;
...
public class Griffin {
        ...
        if (cacher.cacheExists() && !rebuild) {
           ...
           // Data.fileQueue.addAll(qu);
            ...
            if (fastParse && !rebuild) {
                ...
                String path=DirectoryCrawler.SOURCE_DIRECTORY;
                crawler.readIntoQueue(Paths.get(path).normalize());
            }
        }        
    ...
    private static void initializeConfigurationSettings(Path path, String name) throws NumberFormatException, IOException {
    ...
    private static void finalizeConfigurationSettings(String nam, String auth, String tag, String src, String out, String date, String port) {
    ...
    private static void showWelcomeMessage(Path path, String name) {
    ...
    private  static void checkPathValidity(Path path, String name) throws FileSystemException{
        if (!Files.isWritable(path)) {
            ...
            System.out.println("路径不可写入 :(" + LINE_SEPARATOR + "请检查你是否有写入权限");
        ...
        if (Files.exists(path.resolve(name))) {
            ...
            System.out.println("文件已存在 :(" + LINE_SEPARATOR + "请尝试另一个文件名");
        ...
        if (!Files.isDirectory(path)) {
            ...
            System.out.println(" 路径不是一个目录 :(" + LINE_SEPARATOR + "T请尝试另一个路径.");
